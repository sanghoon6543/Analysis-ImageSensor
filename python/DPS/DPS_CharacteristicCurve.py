...
class CharacteristicAnalysis:
    ...
    def Show_ROI(self, ax, Frame):
        ...
        asdf = 1
        self.t_frame = self.ROI_Data.copy()
        self.Label8_4_2.configure(text="PV")
    ...
    def PV2Time(self, Frame, LogFactor, BitDepth, SystemClock):
        ...
        # self.t_frame = Frame
        asdf = 1
        self.Label8_4_2.configure(text="Time")


    def Calculate(self, Frame):

        if self.Label8_4_2.cget("text") == "PV":
            ...
            self.Label8_4_2.configure(text="J")


        Average_ijl = HF.DataProcessing.TemporalAverage(Frame)
        Average_l = HF.DataProcessing.SpatialMedian(Average_ijl, MA=True)
        sigma_s = HF.DataProcessing.SpatialNoise(Average_ijl)
        sigma_t_ijl = HF.DataProcessing.TemporalNoise(Frame)
        sigma_t = HF.DataProcessing.SpatialMedian(sigma_t_ijl, MA=True)

        # WH.Plotting.ShowImage(HF.DataProcessing.TemporalAverage(Frame), ax)
        self.Label8_2_1.configure(text=f"{np.format_float_scientific(Average_l[0], unique=False, precision=2)}")
        self.Label8_2_2.configure(text=f"{np.format_float_scientific(sigma_t[0], unique=False, precision=2)}")

        temp = Average_l[:, np.newaxis]
        self.Output = temp.copy()
        self.Output = np.ma.concatenate((self.Output, sigma_t[:, np.newaxis].copy()), axis=1)
    ...
    def Apply_IQR(self, ax, Frame, NIQR, NIteration, ExcZero, MaskExisting, PrevMask):
        ...
        Average_ijl = HF.DataProcessing.TemporalAverage(IQR_Frame)
        Average_l = HF.DataProcessing.SpatialMedian(Average_ijl, MA=True)
        sigma_s = HF.DataProcessing.SpatialNoise(Average_ijl, Differential=False)
        sigma_t_ijl = HF.DataProcessing.TemporalNoise(IQR_Frame, Differential=False)
        sigma_t = HF.DataProcessing.SpatialMedian(sigma_t_ijl, MA=True)

        WH.Plotting.ShowImage(Average_ijl[0], ax)
        self.Label8_2_1.configure(text=f"{np.format_float_scientific(np.ma.median(Average_l[0]), unique=False, precision=2)}")
        # self.Label8_2_2.configure(text=f"{np.format_float_scientific(np.ma.median(Sigma), unique=False, precision=2)}")
        self.Label8_2_2.configure(text=f"{np.format_float_scientific(sigma_t[0], unique=False, precision=2)}")

        self.Output = np.ma.concatenate((self.Output, Average_l[:, np.newaxis]), axis=1)
        self.Output = np.ma.concatenate((self.Output, sigma_t[:, np.newaxis]), axis=1)

        self.Label9_4_2.configure(text=f"{np.ma.count_masked(Average_ijl[0])}")
    ...
    def _build_ui(self):
        ...
        LabelInfos = [
            ...
            {"parent": self.CalculateFrame, "name": "Label8_4_1", "text": "dtype", "row": 5, "col": 0},
        ...
        ComboBoxInfos = [
            {"parent": self.ReadFrame, "name": "ReadCBox", "textvariable": self.ReadType, "values": [" Open Folders", " Open Files"], "default": " Open Folders", "row": 1, "col": 0, "width": 8, "state": "readonly"},
        ...
        ButtonInfos = [
            ...
            {"parent": self.CalculateFrame, "name": "Button8_3_2", "text": "Use Previous", "command": lambda: self.Apply_IQR(self.ROIWidget, self.t_frame.copy(), self.NIQR.get(), self.NIteration.get(), self.ExcludingZero.get(), self.PrevMask.get(), self.SpatialMask), "row": 4, "col": 1,
             "state": "disabled"},
        ...
        CheckButtonInfos = [
            ...
            {"parent": self.SaveFrame, "name": "CheckButton10_3", "text": "Flip Data", "variable": self.FlipOutput, "row": 3, "col": 0},
        ]
